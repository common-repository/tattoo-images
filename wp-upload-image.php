...
if (isset($_POST['gallery_upload'])) {
	if ( !isset($_POST['post_id']) ) die('Cheating?');
	$post_id = $_POST['post_id'];
	if ( !is_numeric( $post_id ) ) die('Cheating?');

	# Function from wp-pluggable-for-gallery.php is imported from pluggable.php
	...
	require_once('wp-pluggable-for-gallery.php');

	if ( !current_user_can('edit_post', $post_id) ) die('Cheating?');

	$source_url = $_POST['source_url'];
	$tags = $_POST['tags'];
	$page_url = $_POST['page_url'];
	$web_url = $_POST['img_url'];
	...
	$image_upload_path = $wp_upload_dir['path'] . '/gallery/'.$hash_upload[0];
    ...
    $attachment_caption = 'Credit  <a href="'.$web_url.'">'.$image_user.'</a>';
