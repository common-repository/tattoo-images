jQuery(document).ready(function($) {
    ...
    $(document).on('click', '.insert_640', function() {
        var image_src = $(this).data("image_src"),
        	...
        	img_url = $(this).data("web_url");
 
        $('#lightbox').show();
        $('#imgsource').attr('src', ajax_image);
        $('#content').html('<img class="ajaxloading" src="' + ajax_image + '" />');

        $.post('.', { gallery_upload: "1", source_url: image_src, tags: image_tags, page_url: page_url, image_user: image_user, post_id: post_id, img_url : img_url },
            function(data) {
	...
	$('.open_lightbox').click(function(e) {
		var image_href = $(this).attr("href"),
            ...
            infohtml = '\
                <div id="navigation">\
                    <div style="float:right;margin:11px 15px 0 0">By <a href="http://tattmight.com/users/'+image_user+'/" target="_blank" onclick="var event=arguments[0] || window.event; try { event.stopPropagation(); } catch(e) { window.event.cancelBubble = true; }">'+image_user+'</a> / Full resolution <a href="'+page_url+'" target="_blank" onclick="var event=arguments[0] || window.event; try { event.stopPropagation(); } catch(e) { window.event.cancelBubble = true; }">'+image_width+'x'+image_height+'</a></div>\
                    <a href="#" class="button insert_640" data-media_tab="' + load_media_tab + '" data-post_id="' + post_id + '" data-image_src="' + image_href + '" data-image_tags="' + image_tags + '" data-web_url="' + img_url + '"  data-page_url="' + page_url + '" data-image_user="' + image_user + '"><b>Insert</b></a>\
                    <a href="'+page_url+'" target="_blank" class="button" onclick="var event=arguments[0] || window.event; try { event.stopPropagation(); } catch(e) { window.event.cancelBubble = true; }" title="Full resolution image on Tattmight">Visit page</a>\
