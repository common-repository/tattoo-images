...
function byrev_print_form($script, $request_parameters_info, $gallery_api_request) {
	...
	foreach ($request_parameters_info as $name=>$info):
		...
		if (function_exists($func_form)) {
			$input_name = 'gallery-request['.$name.']';
			$echo .= '<div class="input-form">';
			$echo .= '<div class="input-name">'.$info['title']. '</div>';
			$echo .=  $func_form($input_name, $info['values'], $gallery_api_request[$name]);
			$echo .=  '</div>';
		} else {
	...
	$echo .=  '<div style="clear: both;"></div><input type="hidden" name="tab" value="gallerytab" /><input type="submit" value="Search Images"></form>';
	return $echo;
}
?>